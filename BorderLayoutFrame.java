...
public class BorderLayoutFrame extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent event) {
		// verifica a origem de evento e o painel de conteúdo de layout
		// correspondentemente
		for (JButton button : buttons) {
			if (event.getSource() == button)
				button.setVisible(false); // oculta botão clicado
			else
				button.setVisible(true); // mostra outros botões
		} // fim de for

		layout.layoutContainer(getContentPane()); // painel de conteúdo de
													// layout
